 ...
 *   --spec=     : ne moissonner qu'une surspécialité (periop, alr, vent, sepsis, hemo,
 *                 neurotrauma, obst, ped, arret)
...
// revues où une sortie majeure ne peut pas passer inaperçue : on y regarde tous les
// types d'articles. Abréviations PubMed vérifiées sur le catalogue NLM.
const MAJEURES = [
  'N Engl J Med', 'Lancet', 'JAMA', 'BMJ', 'Nat Med', 'NEJM Evid',
  'Lancet Respir Med', 'Intensive Care Med', 'Am J Respir Crit Care Med',
  'Crit Care Med', 'Anesthesiology', 'Br J Anaesth',
];
// revues de surspécialité : on n'y retient que les essais, recos et méta-analyses
const SPECIALISEES = [
  'Anaesthesia', 'Anesth Analg', 'Eur J Anaesthesiol', 'Anaesth Crit Care Pain Med',
  'Ann Intensive Care', 'Crit Care', 'Crit Care Explor', 'Chest', 'Eur Respir J',
  'Resuscitation', 'Ann Emerg Med',
  'Reg Anesth Pain Med', 'J Clin Anesth',
  'Neurocrit Care', 'J Neurotrauma', 'J Trauma Acute Care Surg',
  'Int J Obstet Anesth', 'Am J Obstet Gynecol',
  'Paediatr Anaesth', 'Pediatr Crit Care Med',
  'Ann Surg', 'JAMA Surg', 'Lancet Infect Dis',
];
const TYPES_FORTS = [
...
const SPECS = {
  periop: {
    nom: 'Anesthésie périopératoire',
    termes: ['perioperative', 'preoperative assessment', 'postoperative complications',
             'general anesthesia', 'intraoperative hypotension', 'depth of anesthesia',
             'neuromuscular blockade', 'postoperative delirium', 'myocardial injury after noncardiac surgery',
             'enhanced recovery after surgery', 'perioperative myocardial infarction', 'anesthetic technique'],
  },
  alr: {
    nom: 'Anesthésie locorégionale & douleur',
    termes: ['regional anesthesia', 'nerve block', 'spinal anesthesia', 'epidural analgesia',
             'postoperative pain', 'opioid', 'multimodal analgesia', 'local anesthetic',
             'erector spinae plane block', 'chronic postsurgical pain', 'ketamine analgesia'],
  },
  vent: {
    nom: 'Ventilation, SDRA & oxygénation',
    termes: ['acute respiratory distress syndrome', 'mechanical ventilation', 'tidal volume',
             'positive end-expiratory pressure', 'prone position', 'tracheal intubation',
             'weaning from mechanical ventilation', 'extubation', 'high-flow nasal oxygen',
             'noninvasive ventilation', 'extracorporeal membrane oxygenation', 'oxygen therapy target',
             'tracheostomy', 'acute hypoxemic respiratory failure'],
  },
  sepsis: {
    nom: 'Sepsis & infections graves',
    termes: ['sepsis', 'septic shock', 'antibiotic therapy critically ill', 'antimicrobial duration',
             'ventilator-associated pneumonia', 'community-acquired pneumonia severe',
             'corticosteroids sepsis', 'vasopressor septic shock', 'source control',
             'candidemia', 'bloodstream infection', 'procalcitonin'],
  },
  hemo: {
    nom: 'Hémodynamique, choc & remplissage',
    termes: ['fluid resuscitation', 'balanced crystalloid', 'albumin critically ill',
             'vasopressor', 'norepinephrine', 'vasopressin', 'angiotensin II shock',
             'cardiogenic shock', 'hemorrhagic shock', 'transfusion threshold',
             'red blood cell transfusion', 'platelet transfusion', 'plasma transfusion',
             'tranexamic acid', 'coagulopathy', 'viscoelastic testing', 'prothrombin complex',
             'hemodynamic monitoring', 'cardiac output monitoring', 'fluid balance critically ill',
             'mechanical circulatory support', 'acute kidney injury renal replacement'],
  },
  neurotrauma: {
    nom: 'Neuroréanimation & traumatologie',
    termes: ['traumatic brain injury', 'intracranial pressure', 'cerebral perfusion pressure',
             'subarachnoid hemorrhage', 'intracerebral hemorrhage critical care', 'status epilepticus',
             'polytrauma', 'major trauma', 'trauma resuscitation', 'massive transfusion',
             'damage control resuscitation', 'hypertonic saline', 'decompressive craniectomy',
             'spinal cord injury', 'neuroprognostication', 'neurological outcome after',
             'anesthesia for thrombectomy', 'neurosurgical anesthesia'],
  },
  obst: {
    nom: 'Anesthésie obstétricale',
    termes: ['obstetric anesthesia', 'obstetric anaesthesia', 'cesarean delivery anesthesia',
             'caesarean section anaesthesia', 'neuraxial anesthesia cesarean',
             'labor epidural analgesia', 'labour analgesia', 'spinal hypotension cesarean',
             'postpartum hemorrhage', 'postpartum haemorrhage', 'obstetric hemorrhage',
             'uterotonic', 'placenta accreta', 'severe preeclampsia', 'preeclampsia critical care',
             'maternal critical care', 'maternal sepsis', 'amniotic fluid embolism',
             'enhanced recovery after cesarean'],
  },
  ped: {
    nom: 'Anesthésie-réanimation pédiatrique',
    termes: ['pediatric anesthesia', 'paediatric anaesthesia', 'pediatric intensive care',
             'paediatric intensive care', 'critically ill children', 'neonatal resuscitation',
             'neonatal intensive care ventilation', 'pediatric sepsis', 'pediatric septic shock',
             'pediatric airway management', 'pediatric sedation', 'pediatric trauma',
             'pediatric acute respiratory distress', 'congenital heart surgery anesthesia',
             'bronchiolitis respiratory support', 'caudal block children',
             'anesthesia neurotoxicity developing brain', 'pediatric postoperative pain'],
  },
  arret: {
    nom: 'Arrêt cardiaque, urgences & pré-hospitalier',
  ...
  sedation: {
    ...
    termes: ['sedation in the intensive care unit', 'sedation mechanically ventilated',
             'dexmedetomidine', 'propofol sedation', 'analgosedation', 'sedation depth',
             'delirium critically ill', 'delirium intensive care', 'antipsychotic delirium',
             'sleep in the intensive care unit', 'early mobilization critically ill',
             'rehabilitation after critical illness', 'post-intensive care syndrome',
             ...
             'family in the intensive care unit'],
  },
};
...
const attendues = [];
const SIGLE_CONGRES = /^(SFAR|ESAIC|ESICM|ASA|SCCM|ISICEM|ATS)\s+\d{4}$/;
 ...
 * recommandations traînent un sous-titre à rallonge (« Guidelines of the European Society… »).
...
// sigles d'essai (PROSEVA, TTM-2, CLOVERS…) : majuscules, chiffres et tirets, 3 signes ou plus
const SIGLES_BANALS = new Set(['SFAR', 'ESAIC', 'ESICM', 'ASA', 'SCCM', 'ISICEM', 'ATS', 'ERC', 'ILCOR',
  'NEJM', 'JAMA', 'BJA', 'ICM', 'CCM', 'SSC', 'NICE', 'WHO', 'OMS',
  'ARDS', 'SDRA', 'ICU', 'PICU', 'NICU', 'PEEP', 'CPAP', 'NIV', 'HFNO', 'ECMO', 'ECPR', 'CPR', 'RCP',
  'TBI', 'ICP', 'SAH', 'GCS', 'AKI', 'CRRT', 'RRT', 'SOFA', 'APACHE', 'SAPS', 'COVID', 'ETCO', 'MINS',
  'ERAS', 'PONV', 'TIVA', 'TCI', 'BIS', 'TOF', 'VAP', 'CAP', 'HAP', 'CVC', 'MRI', 'IRM', 'CT', 'PET']);
// TTM-II, TTM II et TTM 2 désignent le même essai : on compare sur la racine
