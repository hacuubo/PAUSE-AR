#!/usr/bin/env python3
# -----------------------------------------------------------------------------
# Pause AR — envoi du bulletin hebdomadaire par e-mail.
#
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;background:#fcfcfb;border:1px solid rgba(11,11,11,.10);border-radius:14px;padding:24px 22px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;color:#111">
  <tr><td>
    <div style="font-size:11px;letter-spacing:.09em;text-transform:uppercase;color:#898781;font-weight:700">Bulletin hebdomadaire &middot; veille en anesth&eacute;sie-r&eacute;animation</div>
    <div style="font-size:24px;font-weight:800;margin-top:4px;letter-spacing:-.01em">PAUSE AR <span style="color:#d03b3b">&#10084;</span></div>
...
def corps_texte(entrees, date_iso):
    lignes = [f"Pause AR — nouveautés de la semaine du {en_francais(date_iso)}", ""]
    for e in entrees:
...
def main():
    ...
    nb_une = sum(1 for e in entrees if e['une'])
    if n:
        sujet = f"Pause AR · {n} nouveauté{'s' if n > 1 else ''} — {en_francais(date_iso)}"
        if nb_une:
            sujet += f" (dont {nb_une} ★ changement de pratique)"
    else:
        sujet = f"Pause AR · bulletin du {en_francais(date_iso)}"
    ...
    msg = EmailMessage()
    msg['Subject'] = sujet
    msg['From'] = f"Pause AR <{expediteur or 'veille@exemple.fr'}>"
